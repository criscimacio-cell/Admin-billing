          ...
          <tbody>
            {logs.map((l) => (
              <tr key={l.id} className="border-b border-slate-100 align-top">
                ...
                <td className="py-2 pr-4">{l.actor_name || '—'}</td>
                <td className="py-2 pr-4 font-mono text-xs">{l.action}</td>
                <td className="py-2 pr-4 text-xs text-slate-500">{l.target_type} · {l.target_id}</td>
            ...
            {logs.length === 0 && (
              <tr><td colSpan={5} className="py-3 text-slate-400">No audit events yet.</td></tr>
            )}
          </tbody>
        </table>
      </Card>
